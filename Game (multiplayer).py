...
Music_List = ['LHS-RLD-10','LHS-RLD-09','RELOADED Memoria']
Music = random.sample(Music_List, 1)
Bg_Music = pygame.mixer.music.load('Assets\\'+str(Music[0])+'.mp3')
...
Finish_Him = pygame.mixer.Sound(os.path.join('Assets','Finish_Him.mp3'))
    ...
    while run: # Prevent pygame from instantly quitting
        ...
        for Event in pygame.event.get(): #Making sure the game quits when asked to (when we click the 'X' icon)
            ...
            if Event.type == pygame.KEYDOWN:
                if Event.key == pygame.K_r and len(Yel_B)<10:
                    bullet = pygame.Rect(Yellow.x+Yellow.width, Yellow.y + (40/2)+4 , 10 , 5)
                    Shoot_Sound.play()
                    Yel_B.append(bullet)
                
                if Event.key == pygame.K_RCTRL and len(Red_B)<10:
            ...
            if Event.type == Red_Hit:
                ...
                if Red_Health==2 or Yellow_Health==2:
                    Bg_Music_handler.set_volume(0.1)
                    Finish_Him.play()
...
################################################################################        
        ...
        if Keys_Pressed[pygame.K_d] and Yellow.x + 5 <= Border.right-55:
            Yellow.x+=5

        if Keys_Pressed[pygame.K_w] and Yellow.y - 5 >= 0:
            Yellow.y-=5

        if Keys_Pressed[pygame.K_s] and Yellow.y + 5 <= 500-40:
            Yellow.y+=5
################################################################################
        ...
        '''
        if Keys_Pressed[pygame.K_r] and len(Yel_B)<10:
            bullet = pygame.Rect(Yellow.x+Yellow.width, Yellow.y + (40/2)+4 , 10 , 5)
            Shoot_Sound.play()
            Yel_B.append(bullet)
                
        if Keys_Pressed[pygame.K_RCTRL] and len(Red_B)<10:
            bullet = pygame.Rect(Red.x, Red.y + (40/2)+4 , 10 , 5)
            Shoot_Sound.play()
            Red_B.append(bullet)
        '''
        pygame.time.Clock().tick(240) # FPS, No of times the display rereashes itself
