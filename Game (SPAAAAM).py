...
Bg_Music = pygame.mixer.Sound(os.path.join('Assets','LHS-RLD-10.mp3'))
...
def main():
    ...
    while run: # Prevent pygame from instantly quitting
        ...
        i+=1
        '''
        if i % 0.125 == 0.0 and i>=100:# and len(Yel_B)<4:
...
################################################################################
        ...
        L = [10,20,30,40,50,60]
        Random1 = random.sample(L, 1)
        Random2 = random.sample(L, 1)
        Random3 = random.sample(L, 1)
        Random4 = random.sample(L, 1)
